...
class TestLayerNorm:
    ...
    @pytest.mark.parametrize("normalized_shape,batch_size,seq_len", [
        (256, 2, 10),
        (512, 1, 5),
        (128, 4, 20),
        (64, 8, 15),
    ...
    def test_layernorm_basic(self, normalized_shape, batch_size, seq_len):
        ...
        if isinstance(normalized_shape, list):
            # Multi-dimensional: normalize over the last len(normalized_shape) dimensions
            norm_dims = list(range(-len(normalized_shape), 0))
        else:
            # Single dimension: normalize over the last dimension
            norm_dims = [-1]
            
        # Check normalization properties
        assert torch.allclose(output.mean(dim=norm_dims, keepdim=True), 
                             torch.zeros_like(output.mean(dim=norm_dims, keepdim=True)), 
                             atol=1e-5)
        assert torch.allclose(output.std(dim=norm_dims, keepdim=True), 
                             torch.ones_like(output.std(dim=norm_dims, keepdim=True)), 
                             atol=1e-2)

    @pytest.mark.parametrize("elementwise_gain,elementwise_bias", [
    ...
    def test_layernorm_weight_bias_combinations(self, elementwise_gain, elementwise_bias):
        ...
        ln = LayerNorm(normalized_shape, 
                      ...
                      elementwise_bias=elementwise_bias)
...
class TestLPLayerNorm:
    ...
    @pytest.mark.parametrize("normalized_shape,dtype", [
        (256, torch.float32),
        (512, torch.float32),
...
class TestRmsNorm:
    ...
    @pytest.mark.parametrize("normalized_shape", [
        256,
        512,
        [64, 4],
        [32, 8, 2],
...
class TestNormClassFactory:
    ...
    @pytest.mark.parametrize("norm_type,expected_class", [
    ...
    @pytest.mark.parametrize("norm_type,has_weight,has_bias", [
    ...
    @pytest.mark.parametrize("invalid_norm", [
        "unsupported_norm",
        "invalid_layernorm",
        "wrong_norm_type",
    ])
...
class TestNormNumericalStability:
    ...
    @pytest.mark.parametrize("norm_class,normalized_shape", [
        (LayerNorm, 256),
        (LPLayerNorm, 256),
        (RmsNorm, 256),
    ])
