...
class TestAttentionFunctions:
    ...
    @pytest.mark.parametrize("batch_size,n_heads,q_seq_len,k_seq_len", [
    ...
    def test_get_rectangular_causal_mask(self, batch_size, n_heads, q_seq_len, k_seq_len):
        ...
        expected_k_len = (k_seq_len + 7) // 8 * 8
    ...
    @pytest.mark.parametrize("batch_size,seq_len,mask_last_n", [
    ...
    @pytest.mark.parametrize("batch_size,seq_len,n_heads,head_dim", [
        (1, 4, 2, 8),      # Minimal
        (2, 8, 4, 16),     # Standard
        (1, 16, 8, 32),    # Larger
        (4, 32, 12, 64),   # Large
    ...
    @pytest.mark.parametrize("batch_size,q_seq_len,k_seq_len,n_heads,head_dim", [
    ...
    @pytest.mark.parametrize("attn_name,extra_args", [
        ("auto", {}),
        ("torch_attn", {}),
...
class TestAttentionNumericalStability:
    ...
    def test_attention_batch_consistency(self, batch_size):
        ...
        if batch_size > 1:
            ...
            single_output = torch_attn(
                queries[:1], keys[:1], values[:1], is_causal=True
            )
