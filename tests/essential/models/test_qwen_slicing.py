...
def _make_qwen_style_inputs_variable_resolution(batch_size, grids_per_sample):
    ...
    """
    images_per_sample = len(grids_per_sample)
    grid_rows = []
    total_patches = 0
...
class TestQwenSlicing:
    ...
    def test_standard_pixel_values_with_image_grid_thw(self):
        """If pixel_values IS batch-indexed but image_grid_thw exists, don't use Qwen path."""
        handler = TransformerBatchHandler()
        batch_size = 4
        inputs = {
            "input_ids": torch.randint(0, 1000, (batch_size, 32)),
            "pixel_values": torch.randn(batch_size, 3, 224, 224),
            # image_grid_thw present but pixel_values has batch dim
            "image_grid_thw": torch.tensor([[1, 16, 16]] * (batch_size * 2), dtype=torch.long),
        }

        sliced = handler.slice_inputs_for_accumulation(inputs, 0, 2)
        # pixel_values should be sliced by batch dim, not by Qwen path
        assert sliced["pixel_values"].shape == (2, 3, 224, 224)
