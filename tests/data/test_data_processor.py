...
class TestProcessorPaliGemma:
    def test_processor_paligemma(self):
        ...
        assert result['attention_mask'].shape == result['input_ids'].shape, "attention_mask should match input_ids shape"
        ...
        assert torch.all((result['attention_mask'] == 0) | (result['attention_mask'] == 1)), "attention_mask should only contain 0s and 1s"
