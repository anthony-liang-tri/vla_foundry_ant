...
def test_get_datastring_input_basic():
    datastrings, num_samples_per_dataset, curr_shard_idx_per_dataset, shard_shuffle_seed_per_dataset = get_datastring_input(
        ...
        manifest_paths = ["tests/shared/dummy_manifest_large.json"],
        dataset_weighting = None,
        allow_multiple_epochs = False,
        num_workers_per_gpu = 4,
        world_size = 8,
    )
    EXPECTED_OUTPUT = 'tests/shared/{00000056,00000043,00000134,00000106,00000186,00000053,00000016,00000069,00000077,00000151,00000018,00000091,00000195,00000065,00000090,00000031,00000141,00000189,00000002,00000081,00000054,00000160,00000198,00000168,00000156,00000087,00000139,00000023,00000041,00000131,00000101,00000172}.tar'
...
def test_get_datastring_input_smalldata_multiple_epochs():
    datastrings_1, num_samples_per_dataset, curr_shard_idx_per_dataset, shard_shuffle_seed_per_dataset = get_datastring_input(
        num_samples = 100_000,
        curr_shard_idx_per_dataset = [0],
        shard_shuffle_seed_per_dataset = [42],
        manifest_paths = ["tests/shared/dummy_manifest_small.json"],
        dataset_weighting = None,
    ...
    for i in range(0, len(datastrings_1_arr), 6):
        if len(datastrings_1_arr[i:i+6]) < 6:
            break
        assert sorted(datastrings_1_arr[i:i+6]) == ['00000000','00000001','00000002','00000003','00000004', '00000005']
    ...
    datastrings_2, num_samples_per_dataset, curr_shard_idx_per_dataset, shard_shuffle_seed_per_dataset = get_datastring_input(
        num_samples = 120_000,
        curr_shard_idx_per_dataset = [0],
        shard_shuffle_seed_per_dataset = [42],
        manifest_paths = ["tests/shared/dummy_manifest_small.json"],
    ...
    for i in range(0, len(datastrings_2_arr), 6):
        if len(datastrings_2_arr[i:i+6]) < 6:
            break
        assert sorted(datastrings_2_arr[i:i+6]) == ['00000000','00000001','00000002','00000003','00000004', '00000005']
    assert num_samples_per_dataset == [148241]
    assert curr_shard_idx_per_dataset == [2]
    assert shard_shuffle_seed_per_dataset == [47]    
    ...
    assert sorted(datastrings_2_arr[-2:] + datastrings_arr[:curr_shard_idx_per_dataset[0]]) == ['00000000','00000001','00000002','00000003','00000004', '00000005']
    for i in range(curr_shard_idx_per_dataset[0], len(datastrings_arr), 6):
        if len(datastrings_arr[i:i+6]) < 6:
            break
        assert sorted(datastrings_arr[i:i+6]) == ['00000000','00000001','00000002','00000003','00000004', '00000005']
    assert num_samples_per_dataset == [148014]
    assert curr_shard_idx_per_dataset == [4]
    assert shard_shuffle_seed_per_dataset == [52]


@pytest.mark.parametrize("num_samples,world_size,param_config_path", [
