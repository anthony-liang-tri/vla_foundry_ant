...
def get_converter(source_type: str, cfg: PreprocessParams) -> BaseRoboticsConverter:
    if source_type == "spartan":
        from vla_foundry.data.scripts.preprocessing.robotics.converters.spartan import SpartanConverter

        return SpartanConverter(cfg)
    elif source_type == "lerobot":
        from vla_foundry.data.scripts.preprocessing.robotics.converters.lerobot import LeRobotConverter
