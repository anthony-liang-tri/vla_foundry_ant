...
def _format_value_for_cmdline(value) -> str | None:
    """
    Format a value for command line argument passing to draccus.

    - None: Returns None (caller should skip this arg — draccus CLI cannot
      parse the string "None" into Python None for Optional fields)
...
def generate_config(
    ...
    if multitask:
        if task is not None:
            raise ValueError("task must be None when multitask=True")
    ...
    for key, value in merged_args.items():
        ...
        # None means the field should use its dataclass default (see _format_value_for_cmdline)
    ...
    try:
        ...
        return (str(output_dir), True, "Generated successfully")

    except subprocess.CalledProcessError as e:
        return (str(output_dir), False, f"Failed: {e.stderr}")
...
def main():
    ...
    tasks = [None] if multitask else ablations_config.get("tasks", nominal["tasks"])
