"""Subprocess entry point for eval rollouts during training.

Invoked by vla_foundry.eval_rollouts via:
    python -m vla_foundry.eval.run_eval_rollouts --experiment-path ... --checkpoint ...
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run eval rollouts for a VLA Foundry checkpoint.")
...
def _draw_label(frame: np.ndarray, text: str) -> np.ndarray:
    ...
    for line_idx, line in enumerate(text.splitlines()):
        ...
        draw.text((x + 1, line_y + 1), line, fill=(0, 0, 0))
        draw.text((x, line_y), line, fill=(255, 255, 255))
        ...
        for frame_idx in range(max_frames):
            ...
            for cell_idx in range(rows * cols):
                if cell_idx < len(selected) and selected[cell_idx]["frames"]:
                    ...
                    label = (
                        f"success {int(video['success'])}\n"
                        f"t {min(frame_idx, len(frames) - 1):03d}/{len(frames) - 1:03d}"
                    )
...
def main():
    ...
    for task in args.tasks:
        ...
        if tracks_coverage:
            ...
            logging.info(
                "[EVAL_ROLLOUTS] task=%s success_rate=%.2f mean_coverage=%.4f",
                task,
                task_success_rate,
                task_mean_coverage,
            )
        else:
            logging.info("[EVAL_ROLLOUTS] task=%s success_rate=%.2f", task, task_success_rate)
