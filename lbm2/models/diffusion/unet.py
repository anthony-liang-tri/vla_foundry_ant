...
class UNet(nn.Module):
    ...
    def __init__(self, model_configs):
        ...
        # Middle
        self.mid_block1 = ResnetBlock(self.channels[-1], self.channels[-1], self.time_emb_dim * self.dim_expansion, self.text_emb_dim)
        self.mid_attn = SelfAttentionBlock(self.channels[-1])
        self.mid_cross_attn = CrossAttentionBlock(self.channels[-1], self.text_emb_dim) if self.text_emb_dim else nn.Identity()
        self.mid_block2 = ResnetBlock(self.channels[-1], self.channels[-1], self.time_emb_dim * self.dim_expansion, self.text_emb_dim)
        ...
        for i, ch in enumerate(reversed(self.channels)):
            ...
            self.up_blocks.append(nn.ModuleList([
                ...
                CrossAttentionBlock(ch, self.text_emb_dim) if (len(self.channels) - i - 1 >= 1 and self.text_emb_dim) else nn.Identity()
            ]))
            if i < len(self.channels) - 1:
        ...
        self.final_conv = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Conv2d(self.channels[0], self.out_channels, 3, padding=1)
        )
