...
class StreamingShardWriter:
    ...
    def add_sample(self, sample: Dict[str, Any], target_image_size: Optional[Tuple[int, int]] = None):
        ...
        # Write action data as NPZ
        action_buf = io.BytesIO()
        np.savez_compressed(action_buf, **sample["actions"])  # Use compression
        action_buf.seek(0)
        info = tarfile.TarInfo(name=f"{sample_id}.actions.npz")
        info.size = len(action_buf.getbuffer())
        self.current_shard_tar.addfile(tarinfo=info, fileobj=action_buf)
