...
from lbm2.data.datasets import SyntheticDatasetUntokenizedText
...
def save_dataset_to_webdataset(dataset, samples_per_shard=1000):
    ...
    for i, (seq,) in enumerate(dataloader):
        ...
        if shard_num > current_shard:
            ...
            manifest_data.append({"shard": f"shard-{current_shard:06d}", "num_sequences": samples_in_current_shard})
        ...
        try:
            ...
            seq_out = seq
        sink.write({"__key__": f"{i:08d}", "json": seq_out})
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create a synthetic dataset in WebDataset format (JSON) and upload to S3"
    )
    ...
    parser.add_argument("--samples-per-shard", type=int, default=512, help="Number of samples per WebDataset shard")
    parser.add_argument("--s3-path", type=str, required=True, help="S3 path (s3://bucket-name/path/to/folder)")

    args = parser.parse_args()

    # Create the dataset
    print(
        f"Creating synthetic dataset with sequence length {args.seq_len}, "
    ...
    temp_dir, shard_files, manifest_file = save_dataset_to_webdataset(dataset, samples_per_shard=args.samples_per_shard)
