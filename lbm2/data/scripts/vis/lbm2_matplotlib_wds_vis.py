...
Single shard (stream) and view everything interactively:
    python lbm2_matplotlib_wds_vis.py s3://my-bucket/path/shard-00001.tar --show

Multiple shards under a prefix (save contact sheets, don't pop windows):
    python lbm2_matplotlib_wds_vis.py s3://my-bucket/path/ --recursive --save --output-dir ./reports

Only print lowdim shapes:
    python lbm2_matplotlib_wds_vis.py s3://my-bucket/path/shard-00001.tar --no-images

Limit images (set -1 to show all):
    python lbm2_matplotlib_wds_vis.py s3://my-bucket/path/shard-00001.tar --max-images 200
