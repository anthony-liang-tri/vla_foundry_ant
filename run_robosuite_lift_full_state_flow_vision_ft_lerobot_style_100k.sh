...
if [ ! -f data/robosuite_lift_full_horizon32/manifest.jsonl ]; then
    python scripts/materialize_lerobot_lift_wds.py \
        --source-root "$LEROBOT_SOURCE_ROOT" \
        --task "lift the red cube off the table" \
        --output-dir data/robosuite_lift_full_horizon32 \
        --past-lowdim-steps 1 \
...
torchrun --nproc_per_node=1 --master_port=29518 \
    vla_foundry/main.py \
    --config_path train_robosuite_flow_lift_full_state_vision_ft_lerobot_style_100k.yaml \
    2>&1 | tee -a outputs/robosuite_flow_lift_full_state_vision_ft_lerobot_style_100k_train.log
